...
public class OVRGazePointer : MonoBehaviour {
	...
	void Update () {
        ...
        transform.position = cameraRig.rightHandAnchor.transform.position + cameraRig.rightHandAnchor.transform.forward * depth;
    ...
    public void SetPosition(Vector3 pos, Vector3 normal)
    {
		...
		depth = (cameraRig.rightHandAnchor.transform.position - pos).magnitude;
    ...
    {
		SetPosition(pos, cameraRig.rightHandAnchor.transform.forward);
    }
    ...
    void LateUpdate()
    {
        ...
        if (positionSetsThisFrame == 0)
        {
            ...
            Quaternion newRot = transform.rotation;
			newRot.SetLookRotation(cameraRig.rightHandAnchor.transform.forward, (lastPosition - transform.position).normalized);
            transform.rotation = newRot;
        }
